...
load("data/ZIM3.Model.AACAGAAA.RData")
...
makeFootprints is used to calculate the aggregate ChIP-exo footprints around given set of binding sites
```{r}
...
plotFootprints is used to plot the aggregate ChIP-exo footprints around any given anchor site.
...
Enuemerating all single variants of the consensus core AACAGAAA
```{r message=FALSE, warning=FALSE}
...
Sorting different cores into three separate classes based on the ChIP-exo footprint profiles
```{r}
...
```{r message=FALSE, warning=FALSE}
## TECookbook package is needed to annotate the genomic binding sites.
require(TECookbook)
data("Repeats.Human.hg38")
Specific.Sites = TECookbook::annotateSitesInRepeat(Specific.Sites, Repeats.Human.hg38, output = "all")
...
require(ggplot2)
Specific.Sites %>% as_tibble() %>%
  tidyr::separate(col = Repeat_Family, sep = "/", into = c("Class", "Repeat_Family")) %>%
  mutate(Class = factor(case_when(Class %in% c("LINE", "SINE", "LTR") ~ as.character(Class),
                                  is.na(Class) ~ "Non-Repeats",
                                  TRUE ~ "Other Repeats"),
...
Specific.Sites %>% as_tibble %>%
  ...
  mutate(Name = paste0(Repeat_Family, "|", Repeat_Name),
         Class = factor(case_when(Class %in% c("LINE", "SINE", "LTR") ~as.character(Class),
                           is.na(Class) ~ "Non-Repeats",
                           TRUE ~ "Other Repeats"),
